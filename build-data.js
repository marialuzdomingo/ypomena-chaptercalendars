...
// Chapter names are normalized before matching (case-insensitive, trailing
// "Integrated" ignored) since Airtable exports have used both forms.
function normalizeKeyPart(s) {
  return s.trim().replace(/\s+integrated$/i, "").toLowerCase();
}

const EVENT_DATE_CORRECTIONS_RAW = {
...
const EVENT_DATE_CORRECTIONS = Object.fromEntries(
  Object.entries(EVENT_DATE_CORRECTIONS_RAW).map(([key, value]) => {
    const [chapter, name] = key.split("|");
    return [`${normalizeKeyPart(chapter)}|${normalizeKeyPart(name)}`, value];
  })
);

function applyEventDateOverride(chapterName, name, isoDate) {
  const key = `${normalizeKeyPart(chapterName)}|${normalizeKeyPart(name)}`;
